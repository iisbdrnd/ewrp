...
<div class="row mt15">
    <div class="col-sm-12">
        ...
        @foreach($galleryImages as $gallery)
              ...
              <li id="{{$gallery->id}}" class="todo-task-item">
                <div class="box nostatus">
                    <img class="img-thumbnail" src="{{url('public/uploads/facilityHeadOffice/thumb/'.$gallery->image_thumb)}}">
                </div>
                <button type="button" class="close todo-close">
                    <i aria-hidden="true" class="icomoon-icon-close" title="remove" id="delete" data="{{$gallery->id}}"></i>
                </button>
            </li>
        @endforeach
        </ul>
    ...
    $(document).ready(function() {
        $( "#sortable" ).sortable({
          beforeStop: function( event, ui ) {
                ...
                $.ajax({
                    url: '{{route("provider.eastWest.gallerySortingAction")}}',
