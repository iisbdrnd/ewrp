<?php $panelTitle = "Operational Team Sorting"; ?>
<style>
    #sortable {
        list-style: none;
...
<form type="update" panelTitle="{{$panelTitle}}" class="form-load form-horizontal group-border stripped" data-fv-excluded="">
                    ...
                    @foreach($teams as $team)
                        <li id="{{$team->id}}" >
                            <div class="box nostatus">
                                <img src="{{asset('public/uploads/managementTeam/thumb')}}/{{$team->image}}"/>
                                <h4 style="width: 100%">
                                    {{$team->name}} 
                                </h4>
    ...
    <div class="row mt15">
        <div class="col-sm-12">
            <a class="btn btn-default btn-sm ajax-link" href="operationalTeam" type="button" style="margin-left: 12px;">Back to List</a>
        </div>
    </div>
</form>
    ...
    $(document).ready(function() {
        $( "#sortable" ).sortable({
          beforeStop: function( event, ui ) {
                ...
                $.ajax({
                    url: '{{route('provider.eastWest.provider.eastWest.operationalTeamSortingAction')}}',
