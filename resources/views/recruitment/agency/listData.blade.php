        ...
        <div class="form-inline">
            <table cellspacing="0" class="responsive table table-striped table-bordered">
                <thead>
                    <tr>
                        <th width="5%">No.</th>
                        <th width="40%" data="1">Agency Name</th>
                ...
                <tbody>
                <?php $paginate = $ewagency; ?>
                @if(count($ewagency)>0)  
                @foreach($ewagency as $agency)
                    <tr>
                        <td>{{$sn++}}</td>
                        ...
                        <td>
                            @if($access->edit)
                                <i class="fa fa-edit" title="Update" view-type="modal" id="edit" data="{{$agency->id}}"></i>
                            @endif
                            @if($access->destroy)
                                <i class="fa fa-trash-o" id="delete" data="{{$agency->id}}"></i>
                            @endif
                        </td>
                        @endif
                    </tr>
