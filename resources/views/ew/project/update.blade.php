...
<form type="update" panelTitle="{{$panelTitle}}" class="form-load form-horizontal group-border stripped">
    ...
    <div class="form-group">
        ...
        <div class="col-lg-9 col-md-8">
            <input autofocus required name="project_name" placeholder="Project Name" class="form-control" value="{{$project->project_name}}">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-lg-9 col-md-8">
            <select required name="trade_id[]" data-fv-icon="false" add-url="tradeAdd" class="@if($tradeAddAccess){{'select2-add'}}@endif select2 form-control ml0" multiple="multiple">
                ...
                <option value="{{$trade->id}}" @if(in_array($trade->id, $selected_trades)) selected @endif>{{$trade->trade_name}}</option>
                @endforeach
            </select>
        </div>
    </div>
