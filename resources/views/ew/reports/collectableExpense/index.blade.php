<?php $panelTitle = "Collectable Expense"; ?>
@include("panelStart")
<div class="row mt15">
    <!-- For Payment Add -->
    <form id="collectableExpenseForm" method="post" type="view" class="form-horizontal" data-fv-excluded="">
        {{csrf_field()}}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("input[name=date_range]").on('change', function() {
            ...
            if (date_range==1) {
                if($("#date_fields").is(':visible')) {
                    ...
                    $('#collectableExpenseForm').formValidation('removeField', $("#from_date"));
                    $('#collectableExpenseForm').formValidation('removeField', $("#to_date"));
                    $("#date_fields").hide();
                }
                $("#upto_date_fields").show();
                $("#date").attr('required', 'required');
                $('#collectableExpenseForm').formValidation('addField', $("#date"));
            } else if (date_range==2) {
                if($("#upto_date_fields").is(':visible')) {
                    $("#date").removeAttr('required');
                    $('#collectableExpenseForm').formValidation('removeField', $("#date"));
                    $("#upto_date_fields").hide();
                }
                $("#date_fields").show();
                $("#from_date").attr('required', 'required');
                $("#to_date").attr('required', 'required');
                $('#collectableExpenseForm').formValidation('addField', $("#from_date"));
                $('#collectableExpenseForm').formValidation('addField', $("#to_date"));
            } else {
                if($("#upto_date_fields").is(':visible')) {
                    $("#date").removeAttr('required');
                    $('#collectableExpenseForm').formValidation('removeField', $("#date"));
        ...
        $("#collectableExpenseForm").formValidation().on('success.form.fv', function(e) {
            e.preventDefault();
            ...
            var url = "{{route('ew.collectableExpenseReport')}}"+'?project='+project+'&group_report='+group_report+'&date_range='+date_range + dateParameter;
