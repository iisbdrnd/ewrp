<?php $panelTitle = "Accounts Payable"; ?>
@include("panelStart")
<div class="row mt15">
    <!-- For Payment Add -->
    <form id="accountsPayableForm" method="post" type="view" class="form-horizontal" data-fv-excluded="">
        {{csrf_field()}}
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#accountsPayableForm').formValidation('removeField', $(".dtpicker"));

        $("#date_range").on('change', function() {
            if ($(this).is(":checked")) {
                $("#date_fields").show();
                $(".dtpicker").attr('required', 'required');
                $('#accountsPayableForm').formValidation('addField', $(".dtpicker"));
                date_range = 1;
            } else {
                $("#date_fields").hide();
        ...
        $("#accountsPayableForm").formValidation().on('success.form.fv', function(e) {
            e.preventDefault();
            ...
            var previewType = 1;
            var url = "{{route('ew.accountsPayableReport')}}"+'?project='+project+ '&date_range='+date_range+ '&date='+date;
