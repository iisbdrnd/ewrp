<?php $panelTitle = "Amount Less"; ?>
@include("panelStart")
<form id="amountLessForm" type="create" panelTitle="{{$panelTitle}}" class="form-load form-horizontal" data-fv-excluded="" callback="refresh_amount_less">
    {{csrf_field()}}
    <div class="row mt15">
    ...
    <div class="col-md-12 btn-pb15">
        <div class="form-group mb0">
            <button type="submit" class="btn btn-default">Save Amount Less</button>
        </div>
    </div>
</form>
@include("panelEnd")
...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            if (ew_project_id) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#amountLessForm').formValidation('removeField', $('#candidate_id'));
                        $("#project_candidate_id").html(data);
                        $("#candidate_id").select2({ placeholder: "Select" });
                        $('#amountLessForm').formValidation('addField', $('#candidate_id'));

                        //Reset
                        $('#amountLessForm').formValidation('removeField', $('#account_head'));
                        ...
                        $('#amountLessForm').formValidation('addField', $('#account_head'));
        ...
        $("#amountLessForm").on('change', '#candidate_id', function(e) {
        ...
        $("#amountLessForm").on('change', '#account_head', function(e) {
            candidateAccountHeadSummary();
    ...
    function remarksGen(){
        ...
        if (project && candidate_id && account_head && amount) {
            var remarksText = account_head+'-'+project+'-'+candidate_id+' Less Amount '+amount+' Tk.';
            $("#remarks").val(remarksText);
        }
    }

    $("#amountLessForm").on('change', '#ew_project_id', function(e) {
        $("#remarks").val();
    });  
    ...
    $('#amountLessForm').submit(function(e) {
        e.preventDefault();
        var projectId = $('#ew_project_id option:selected').val();
        selectProejctId = projectId;
    });

    function refresh_amount_less() {
        ...
        $('#ew_project_id').val(selectProejctId).trigger('change');
