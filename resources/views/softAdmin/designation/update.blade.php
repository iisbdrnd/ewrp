...
<form type="update" data-fv-excluded="" panelTitle="{{$panelTitle}}" class="form-load form-horizontal group-border stripped">
        ...
        <div class="col-lg-10 col-md-9">
            <select  required data-fv-icon="false" name="project_id" id="project_id" class="form-control">
                ...
                @foreach($projects as $project)
                    <option value="{{$project->id}}" @if($project->id==$adminDesignation->project_id)selected<?php $project_name=$project->name; ?>@endif>{{$project->project_id}}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-lg-10 col-md-9">
            <input required name="name" placeholder="Designation Name" class="form-control" value="{{$adminDesignation->name}}">
        </div>
    </div>
    <div class="form-group">
        <label class="col-lg-2 col-md-3 control-label required">Grade</label>
        <div class="col-lg-10 col-md-9">
            <input required name="grade" placeholder="Grade" class="form-control" value="{{$adminDesignation->grade}}">
        </div>
    </div>
