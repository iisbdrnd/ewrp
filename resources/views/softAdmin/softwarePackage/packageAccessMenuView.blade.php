<div class=row>
    <div class="col-lg-12">
        ...
        <div id="packageAccessPanel" class="panel panel-default panelMove showControls toggle panelClose panelRefresh">
            <!-- Start .panel -->
            ...
            <div class="panel-body">
                <form id="package-access-form">
                    ...
                    <div class="row ml0 mt20">
                        {{csrf_field()}}
                        <button id="package-access-save" class="btn btn-default" type="button">Save Access</button>
                    </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#package-access-save").click(function(){
            preLoader($("#packageAccessPanel"));
            var data = $("#package-access-form").serializeArray();
            var module_id = $(".stats-btn-selected").attr('data');
            data[data.length] = {name:'package', value:$("#package_id").val()};
            data[data.length] = {name:'module', value:module_id};
            $.ajax({
                url: "{{route('softAdmin.packageAccess')}}",
                type: "POST",
                data: data,
                success: function (data) {
                    ...
                    preLoaderHide($("#packageAccessPanel"));
                    packageAccessView(module_id);
                }
            });
        });
    });
