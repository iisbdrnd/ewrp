		...
		@if(count($users)>0)
			@foreach($users as $user)
				<tr>
					<td>{{$sn++}}</td>
					<td>{{$user->emp_id}}</td>
                    <td>@if(Helper::adminAccess('user.show'))<a href="user/{{$user->id}}" menu-active="user" class="ajax-link hand" data-toggle="tooltip" data-placement="right" data-html="true" title="{{$user->name}}">{{$user->name}}</a>@else{{$user->name}}@endif</td>
