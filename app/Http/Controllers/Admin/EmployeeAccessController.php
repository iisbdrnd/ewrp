...
use App\Model\SoftwareMenuAccess_user;
...
class EmployeeAccessController  extends Controller
{
    public function employeeAccess(Request $request)
    {
        ...
        $data['emp_info'] = User_user::valid()->find($emp_id);
        $project_id = Auth::user()->get()->project_id;
        $data['software_modules'] = collect(SoftwareModules::projectAccessModules($project_id))->chunk(4);

        return view('admin.employee.employeeAccess', $data);
    }
    ...
    public function employeeAccessMenuView(Request $request)
    {
        ...
        if(!empty($data['software_module'])) {
            ...
            return view('admin.employee.employeeAccessMenuView', $data);
        }
    }
    ...
    public function employeeAccessAction(Request $request)
    {
        ...
        $menu_db = collect(SoftwareMenuAccess_user::select('software_menu_access.*')
            ...
            ->get()->pluck('menu_id'));
        $link_db = collect(SoftwareInternalLinkAccess_user::select('software_internal_link_access.*')
        ...
        if(!empty($menu_diff)) {
            $cur_menu_ac = SoftwareMenuAccess_user::whereIn('menu_id', $menu_diff)->where('user_id', $user_id)->get();
            foreach($cur_menu_ac as $cur_menu) {
                SoftwareMenuAccess_user::find($cur_menu->id)->delete();
            }
        }
        if(!empty($link_diff)) {
            $cur_link_ac = SoftwareInternalLinkAccess_user::whereIn('link_id', $link_diff)->where('user_id', $user_id)->get();
            foreach($cur_link_ac as $cur_link) {
                SoftwareInternalLinkAccess_user::find($cur_link->id)->delete();
            }
        }
        ...
        if(!empty($menu)) {
            ...
            foreach($menu as $menu_id) {
                if($projectAccess_menu->contains($menu_id) && !$menu_db->contains($menu_id)) {
                    ...
                    SoftwareMenuAccess_user::create(array(
                        'menu_id' => $menu_id,
                        'user_id' => $user_id
                    ));
                }
        ...
        if(!empty($link)) {
            ...
            foreach($link as $link_id) {
                if($projectAccess_link->contains($link_id) && !$link_db->contains($link_id)) {
                    SoftwareInternalLinkAccess_user::create(array(
                        'link_id' => $link_id,
                        'user_id' => $user_id
                    ));
                }
    ...
    public function employeeAccessView(Request $request)
    {
        ...
        return view('admin.employee.employeeAccessView', $data);
    }
