...
class LedgerQueryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //return response($data['accountLevelOfFour']);
        //die();
        return view('ew.reports.ledgerQuery.index', $data);
    }
    ...
    public function ledgerQuery(Request $request) {
        ...
        $data = self::getLedgerQuery($param);
        $pdf_url = route('ew.ledgerQueryPDF').'?project='.$project_id.'&account='.$account_code.'&date_range='.$date_range.'&from_date='.$from_date.'&to_date='.$to_date;

        $data = array_merge($data, ['pdf_url' => $pdf_url]);
        return view('ew.reports.ledgerQuery.report', $data);
    }

    public function ledgerQueryPDF(Request $request) {
        ...
        $pdf = PDF::loadView('ew.reports.ledgerQuery.report', $data);
        $file_name = 'ledger-query-'.date('Y-m-d').'.pdf';
        return $pdf->stream($file_name);
    }

    public static function getLedgerQuery($param) {
