...
class AmountRefundController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        return view('ew.amountRefund.create', $data);
    }
            ...
            if($amount>0) {
                ...
                if(!empty($candidate)) {
                    if($candidate->flight_status==0) {
                        ...
                        if(($transaction_balance->received_amount-$transaction_balance->paid_amount)>=$amount) {
                            $transaction_date = date('Y-m-d');
                            $transaction_status = ($bankPayment)?3:1;
                            ...
                            $account_input = [
                                ...
                                'voucher_type'          => ($bankPayment)?'BPV':'PV',
                                'collectable_account'   => $request->account_head,
                                'transaction_by'        => $transaction_by,
                                'transaction_date'      => $transaction_date,
                                'remarks'               => $request->remarks,
                            ...
                            $output['messege'] = 'Account refund has been completed';
                            $output['msgType'] = 'success';
                        } else {
                            $output['messege'] = 'Amount can not be greater than received amount';
