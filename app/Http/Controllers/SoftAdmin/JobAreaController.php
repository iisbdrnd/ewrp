...
class JobAreaController extends Controller
{
    ...
    public function jobAreaListData(Request $request) {
        ...
        $data['jobArea'] = JobArea_admin::leftJoin('project', function($join)
            {
                $join->on('job_area.project_id', '=', 'project.id')
                     ->on('project.valid', '=', DB::raw(1));
            })
            ->select('job_area.*', 'project.project_id as project_id_view', 'project.name as project_name')
            ->where('job_area.valid', 1)
            ->where(function($query) use ($search)
            {
                $query->where('job_area.area_name', 'LIKE', '%'.$search.'%')
                    ->orWhere('job_area.updated_at', 'LIKE', '%'.$search.'%');
            })
    ...
    {
        $data['adminJobArea'] = JobArea_admin::valid()->find($id);
        $data['projects'] = Project::where('valid', 1)->orderBy('project_id', 'asc')->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($validator->passes()) {
        JobArea_admin::valid()->find($id)->update($input);
    ...
    {
        JobArea_admin::valid()->find($id)->delete();
    }
}
