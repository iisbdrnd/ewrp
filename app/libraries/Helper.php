...
class Helper {
    ...
    public static function userModuleAccess($module_prefix){
        $user_id = Auth::user()->get()->id;
        ...
        foreach ($modules as $key => $value) {
            if($value->url_prefix == $module_prefix){
                $result = true;
            }
        }
        return $result;
    ...
    public static function getCandidateBalance($candidate_id, $account_head=0) {
        ...
        if($account_head>0) {
            ...
            $receivable_amount = $data['receivable'] = EwCandidateTransaction::valid()
        ...
        $transaction->balance = $receivable_amount - ($transaction->received_amount+$transaction->less_amount-$transaction->paid_amount);
        return $transaction;
    }
    ...
    public static function companyDetails() {
        ...
        $html = '<p class="company_logo"><img src="'.asset("public/uploads/logo/$companyDetails->logo").'" style="width:120; height:60"></p>
            <p class="company_name">'.$companyDetails->company_name.'</p>
    ...
    public static function deployDate($projectId, $candidateId){
        $data = EwMobilizationMasterTable::where('ew_project_id', $projectId)->where('ew_candidatescv_id', $candidateId)
            ->where('flight_completed', 1)
            ->first();
        $date = date('Y-m-d', strtotime($data->flight_date));
        return $date;
    }
...
/* Mobilization complateness status */
    ...
    */
    ...
    public static function  companyLogo($width,$height) {
        ...
        $project_id = Auth::user()->get()->project_id;
        $companyDetails= Project::join('project_info', 'project.id', '=', 'project_info.project_id')
            ->select('project_info.logo')
            ->where('project.id', $project_id)
            ->first();

        $url = asset("public/uploads/logo/$companyDetails->logo");
        $html = '<center>
    ...
    public static function candidateAPVQuantity($project_id, $trade_id){
        $data = EwCandidatesCV::valid()
            ->where('ew_project_id', $project_id)
            ->where('selected_trade', $trade_id)
            ->where('approved_status', 1)
            ->where('result', 1)
            ->count();
        return $data;
    ...
    public static function candidateFitQuantity($project_id, $trade_id){
        
        $data = EwCandidatesCV::join('ew_mobilization_master_tables', 'ew_mobilization_master_tables.ew_candidatescv_id', '=', 'ew_candidatescv.id')
        ->select('ew_candidatescv.id', 'ew_candidatescv.selected_trade','ew_mobilization_master_tables.medical_actual_status')
        ->where('ew_candidatescv.ew_project_id', $project_id)
        ->where('ew_candidatescv.selected_trade', $trade_id)
