...
function panelRefresh(that, refreshUrl, data, refreshCallBack) {
    ...
    $.ajax({
        ...
        url : URL.getSiteAction('/'+refreshUrl),
        data: data,
        type: 'GET',
        dataType: "html",
        ...
        selector.find(".data-list")
            ...
            .on('click', "#delete", function(e) {
                ...
                swal({
                    ...
                    preLoader(that);
                    $.ajax({
                        url : URL.getSiteAction('/'+deleteLinkUrl),
                        type: "DELETE",
                        data: {'_token': csrf},
                        dataType: "html",
                        success:function(data){
...
function loadDataListForm(that, url, data, viewType, modalData, backPermit, callBack, extraParameter) {
    ...
    if(viewType=='modal') {
        ...
        bootbox.dialog({
            message: '<div id="add-details"><div class="text-center"><img src="'+URL.baseUrl('/public/img/loaders/horizontal/006.gif')+'" /></div></div>',
            title: title,
            size: modalSize,
            buttons: {
                close: {
...
function formSetup(that, type, callBack, url, extraParameter) {
    ...
    }).on('success.form.fv', function(e)
    {
        ...
        if(typeof formURL_attr!==typeof undefined && formURL_attr!==false) {
            ...
            mainUrl = urlSplit[0];
            var formURL = URL.getSiteAction('/'+mainUrl);
        }
                    ...
                    if(data.msgType=='success') {
						...
						if(typeof extraParameter!==typeof undefined && extraParameter.viewType=='modal' && typeof extraParameter.returnSelector!==typeof undefined) {
                            ...
                            if(extraParameter.returnUrl!==typeof undefined) {
                                $.ajax({
                                    url: URL.getSiteAction('/'+extraParameter.returnUrl),
                                    type: "GET",
                                    dataType: "html",
                                    success: function(data) {
...
function fileUpload(selector)
{
    ...
    if(preFile) {
        ...
        if(file_ext == 'doc' || file_ext == 'docx') {
            selector.find($('#'+imgViewId)).html('<div class="single-item" style="width: 150px;"><div class="img"><img src="'+URL.baseUrl('/public/uploads/doc_icon.jpg')+'" class="img-overly" /><div class="overlay upload-img-overly"><h1 title="Remove" class="fa fa-times-circle file-remove hand"></h1></div></div></div>');
        } else if(file_ext == 'pdf'){
            selector.find($('#'+imgViewId)).html('<div class="single-item" style="width: 150px;"><div class="img"><img src="'+URL.baseUrl('/public/uploads/pdf_icon.jpg')+'" class="img-overly" /><div class="overlay upload-img-overly"><h1 title="Remove" class="fa fa-times-circle file-remove hand"></h1></div></div></div>');
        } else {
            selector.find($('#'+imgViewId)).html('<div class="single-item"'+maxWidth+'><div class="img"><img src="'+URL.baseUrl('/'+filePath+'/'+preFile)+'" class="img-overly" /><div class="overlay upload-img-overly"><h1 title="Remove" class="fa fa-times-circle file-remove hand"></h1></div></div></div>');
        }
...
function select2Add(selector) {
	...
	if(typeof select2AddUrl===typeof undefined) { alert('Add URL is undefined'); }
		...
		$.ajax({
			mimeType: 'text/html; charset=utf-8',
			url: URL.getSiteAction('/'+select2AddUrl),
			data: {takeContent:1},
			type: 'GET',
			dataType: "html",
			success: function(data) {
...
function showDetails(title, detailsUrl, data, modalSize) {
    ...
    bootbox.dialog({
        message: '<div id="event-details"><div class="text-center"><img src="'+URL.baseUrl('/public/img/loaders/horizontal/006.gif')+'" /></div></div>',
        title: title,
        size: modalSize,
        buttons: {
            close: {
    ...
    $.ajax({
        url: URL.getSiteAction('/'+detailsUrl),
        data: dataArray,
        type: 'GET',
        dataType: "html",
        success: function(data) {
		...
		if(popoverUrl) {
			...
			$selector.on('show.bs.popover', function () {
				if($selector.attr('load-popover')) {
					$.ajax({
						url: URL.getSiteAction('/'+popoverUrl),
						dataType: 'html',
...
function redirectLoginPage() {
    ...
    function(){
        location.replace(URL.loginPage());
    });
}
