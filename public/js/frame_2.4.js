...
function seqPaginateParameterDestroy(dataPrefix) {
    var seqPaginate = (dataPrefix) ? dataPrefix+"-seq-paginate" : "seq-paginate";
    ...
    if(loadUrl) {
        ...
        selector.find(".data-list")
                        ...
                        success: function(data) {
                            if(parseInt(data)===0) {
                                ...
                                if (that.find(".data-list .form-load").length>0) {
                                    that.find(".panel-title").html(that.find(".data-list form[paneltitle]").attr('paneltitle'));
            ...
            .on('click', ".back-btn", function(e) {
                ...
                that.find(".panel-title").html(panelTitle);
            ...
            .on('click', ".seq-forward", function(e) {
                ...
                if (typeof seqForwardUrl===typeof undefined || seqForwardUrl=='') { alert('Url undefined.') }
                    ...
                    var seqPaginate = (dataPrefix) ? dataPrefix+"-seq-paginate" : "seq-paginate";
                    var inputHtml = '<input panel-title="'+that.find(".panel-title").html()+'" main-selector="'+mainSelector+'" clone-selector="'+cloneSelector+'" class="'+seqPaginate+'" type="hidden" value="'+that.attr('refresh-url');
                    inputHtml += '">';
                    ...
                    if(panelTitle) { $("#"+mainSelector).find(".panel-title").html(panelTitle); }
...
function loadAjaxPaginateContent(that, url, loadUrl, inputArray, dataPrefix, urlParameterManualPermission, callBack, preLoading=true){
    if (typeof urlParameterManualPermission===typeof undefined || urlParameterManualPermission==="") { urlParameterManualPermission=true; }
    if(preLoading) { preLoader(that); }
    $.ajax({
        ...
        success: function(data) {
            if(parseInt(data)===0) {
                ...
                if(preLoading) { preLoaderHide(that); }
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            swal("Cancelled", errorThrown, "error");
        ...
        success: function(data) {
            if(parseInt(data)===0) {
                ...
                if(viewType=='modal') {
                    ...
                    that.find(".panel-title").html(that.find(".data-list form[paneltitle]").attr('paneltitle'));
                    that.find(".data-list form[paneltitle]").removeAttr('paneltitle');
                    if(backPermit) { that.find("button:last").parent().append('<button type="button" class="back-btn btn btn-default ml5">Back to List</button>'); }
                    loadForm(that, url);
                }
...
function formSetup(that, type, callBack, url, extraParameter) {
    //Focus
	var $firstInput = that.find(".form-control").first();
    if($firstInput.hasClass("select2-container")) {
        $firstInput.parent().find(".form-control").select2('focus');
    } else {
                ...
                if(parseInt(data.auth)===0) {
                    ...
                    if(data.msgType=='success') {
						...
						if(typeof extraParameter!==typeof undefined && extraParameter.viewType=='modal' && typeof extraParameter.returnSelector!==typeof undefined) {
                            ...
                            if(extraParameter.returnUrl!==typeof undefined) {
                                ...
                                loadAjaxPaginateContent(extraParameter.returnSelector.parents(".data-table").first(), url, extraParameter.returnUrl, returnInputArray, extraParameter.returnDataPrefix, extraParameter.returnUrlParameterPermission, false, false);
                            }
						}
                    ...
                    if(data.msgType=='success') {
                        ...
                        var validatorMsg = '<div class="alert alert-danger fade in"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'+data.messege+'</div>';
                        if(typeof extraParameter!==typeof undefined && extraParameter.viewType=='modal') {
    						that.parents('.modal-body').prepend(validatorMsg);
    					} else {
    						that.parents('.panel-body').prepend(validatorMsg);
    					}
                    }
					if(typeof extraParameter!==typeof undefined && extraParameter.viewType=='modal') {
						$btn1.button('reset');
    ...
    $(function(){
        ...
        new AjaxUpload(btnUpload, {
            ...
            onSubmit: function(file, ext){
                ...
                img_view.html('<img src="'+appUrl.baseUrl('/public/img/loaders/horizontal/006.gif')+'" alt="" class="mt10" />');
            },
...
function advanceSearch(selector) {
    selector.find(".adv-btn").click(function(){
        ...
        if(searchUrl) {
            ...
            }).on('hidden.bs.modal', function(){ modalScroll(); selector.find("select.adv-search").select2('focus'); });
...
function select2Add(selector) {
	...
	if(typeof select2AddUrl===typeof undefined) { alert('Add URL is undefined'); }
		...
		}).on('hidden.bs.modal', function(){ modalScroll(); selector.select2('focus'); });
...
function centerModal(modalHeight) {
    ...
    if(isCenter) {
        var modalHeight = $lastModal.find(".modal-content").height()-modalHeight;
        if(modalHeight>0) {
            var topMargin = parseInt($lastModal.find(".modal-dialog").css("margin-top"))-(modalHeight/2);
            if(topMargin>0) {
...
$(window).load(function() {
    ...
    currentHash=ajax_url;
    windowAjaxLoad(ajax_url);
});
...
$(document).keydown(function( event ) {
    switch(event.which) {
        //Search
        case 113:
		if($(event.target).parents(".adv-search").length>0) {
			$(event.target).parents(".adv-search").first().parent().find("select").select2("close");
            $(event.target).parents(".advance-search").first().find(".adv-btn").trigger("click");
        } else if($(event.target).hasClass("select2-input")) {
            var selectInputId = $(event.target).attr("id");
            var selectInputId = selectInputId.split("_search");
            if($("#"+selectInputId[0]).parents(".adv-search").length>0) {
				$("#"+selectInputId[0]).parents(".adv-search").first().parent().find("select").select2("close");
                $("#"+selectInputId[0]).parents(".advance-search").first().find(".adv-btn").trigger("click");
            }
        }
        else if($(".data-search").length>0) { $(".data-search").first().focus(); }
		...
		if($(event.target).parents(".select2-container").length>0 && $(event.target).parents(".select2-container").first().parent().find(".select2-add").length>0) {
			var selectInputId = $(event.target).attr("id");
			$("#"+selectInputId+"_search").parents(".select2-drop").first().find(".select2-add-btn>.btn").trigger("click");
		} else if($(event.target).hasClass("select2-input") && $("#"+selectInputId[0]).parents(".select2-container").first().parent().find(".select2-add").length>0) {
			$(event.target).parents(".select2-drop").first().find(".select2-add-btn>.btn").trigger("click");
		} else if($(".modal").length>0) {
			if($(".modal").last().find(".add-btn").length>0) { $(".modal").last().find(".add-btn").first().trigger("click"); }
			else if($(".modal").last().find(".adv-add-btn").length>0) { $(".modal").last().find(".adv-add-btn").first().trigger("click"); }
		} else { $(".add-btn").first().trigger("click"); }
        event.preventDefault();
        break;
        //Cancel/Esc
        case 27:
		if($(".modal").length>0) {
			if($(".modal").last().find(".back-btn").length>0) { $(".modal").last().find(".back-btn").trigger("click"); }
			else { $(".modal").last().modal('hide'); }
		}
        else if($(".back-btn").length>0) { $(".back-btn").trigger("click"); }
        event.preventDefault();
        break;
    }
});
...
function windowAjaxLoad(ajax_url) {
