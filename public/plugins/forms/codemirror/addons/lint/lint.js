...
if(tooltips!=!1)CodeMirror.on(inner,"mouseover",function(e){showTooltipFor(e,labels,inner)});return marker}
...
function startLinting(cm){var state=cm.state.lint,options=state.options;var passOptions=options.options||options;if(options.async)
options.getAnnotations(cm.getValue(),updateLinting,passOptions,cm);else updateLinting(cm,options.getAnnotations(cm.getValue(),passOptions,cm))}
...
CodeMirror.defineOption("lint",!1,function(cm,val,old){if(old&&old!=CodeMirror.Init){clearMarks(cm);cm.off("change",onChange);CodeMirror.off(cm.getWrapperElement(),"mouseover",cm.state.lint.onMouseOver);delete cm.state.lint}
if(val){var gutters=cm.getOption("gutters"),hasLintGutter=!1;for(var i=0;i<gutters.length;++i)if(gutters[i]==GUTTER_ID)hasLintGutter=!0;var state=cm.state.lint=new LintState(cm,parseOptions(cm,val),hasLintGutter);cm.on("change",onChange);if(state.options.tooltips!=!1)
