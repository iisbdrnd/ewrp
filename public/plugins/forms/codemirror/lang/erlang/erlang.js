...
var w=stream.current();if(is_member(w,keywordWords)){return rval(state,stream,"keyword")}else if(is_member(w,operatorAtomWords)){return rval(state,stream,"operator")}else if(stream.match(/\s*\(/,!1)){if(is_member(w,bifWords)&&((peekToken(state).token!=":")||(peekToken(state,2).token=="erlang"))){return rval(state,stream,"builtin")}else if(is_member(w,guardWords)){return rval(state,stream,"guard")}else{return rval(state,stream,"function")}}else if(is_member(w,operatorAtomWords)){return rval(state,stream,"operator")}else if(lookahead(stream)==":"){if(w=="erlang"){return rval(state,stream,"builtin")}else{return rval(state,stream,"function")}}else if(is_member(w,["true","false"])){return rval(state,stream,"boolean")}else if(is_member(w,["true","false"])){return rval(state,stream,"boolean")}else{return rval(state,stream,"atom")}}
