...
$(window).load(function() {
    ...
    if (!mobileDevice) {
        $('.twButton').hover(function() {
            ...
            $('.twContent').stop(true).show().css('opacity', '0').animate({
                opacity: '1'
            }, 0);
            clearTimeout(twtTimer);
            nextTweet();
            $('.twContent').hover(function() {
                $('.twContent').stop(true).animate({
                    opacity: '1'
                }, 0);
            }, function() {
                $('.twContent').stop().delay(300).animate({
                    opacity: '0'
                }, 0, function() {
...
function nextTweet() {
    ...
    $('.twContent ul').animate({
        height: $('.twContent li.active').height() + 10
    }, 500);
...
function initMenu() {
    ...
    $('#menu-container').css({
        left: '-' + ($('#menu-container').outerWidth()) + 'px'
    });
    if (bgPaused) {
        $('#bgControl .play').show();
...
function runBg() {
    ...
    if ($('#bgImages li.active iframe').length > 0) {
        if ($('#bgImages li.active iframe').attr('videotype') == 'youtube') {
            ...
            $('#vmVideo').append($('#bgImages li.active iframe').clone().attr('src', $('#bgImages li.active iframe').attr('src') + '&autoplay=1&loop=0&controls=0&player_id=vimeoplayer').attr('id', 'vimeoplayer'));
    ...
    clearInterval(bgTimer);
    doSize();
    $('#bgText h3, #bgText .subText').stop().animate({
        opacity: '0'
    }, 300, function() {
    ...
    $('#bgImageWrapper .new').stop(true).animate({
        opacity: 1
    }, 500);
...
function pageLoaded() {
    ...
    $('#contentBox').stop().delay(500).animate({
        opacity: '1',
        marginTop: 20
    }, 600, 'easeOutExpo');
...
function setImageAnimations() {
    $('#contentBox .image_frame a img').each(function() {
        ...
        $(this).parent().parent().find('.hoverWrapper h3, .hoverWrapper .enter-text').css({
            opacity: '0'
        });
        $(this).parent().parent().find('.hoverWrapper a').css({
            opacity: '0'
        });
        if ($(this).parent().parent().find('.hoverWrapper h3, .hoverWrapper .enter-text').length == 0)
            $(this).parent().parent().find('.modal, .link, .modalVideo').css({
                bottom: parseInt((($(this).height() - 26) / 2)) + 'px'
            });
        var centerX = parseInt($(this).parent().parent().find('.hoverWrapper').width() / 2);
        $(this).parent().parent().find('.modal, .modalVideo').css({
            left: (centerX - 41) + 'px'
        }).click(function(event) {
            $(this).find('a').trigger('click');
        });
        $(this).parent().parent().find('.link').css({
            left: (centerX + 15) + 'px'
        }).click(function(event) {
...
function imageOver(obj, event, centerX) {
    event.stopPropagation();
    $(obj).find('.modal, .modalVideo').stop().animate({
        opacity: '1',
        left: (centerX - 31)
    }, 100);
    $(obj).find('.link').stop().animate({
        opacity: '1',
        left: (centerX + 5)
    }, 100);
    $(obj).find('.hoverWrapperBg').stop().animate({
        opacity: '.85'
    }, 400);
    $(obj).find('.hoverWrapper h3').stop().delay(100).animate({
        opacity: '1'
    }, 300);
    $(obj).find('.hoverWrapper .enter-text').stop().delay(200).animate({
        opacity: '1'
    }, 300);
}
...
function imageOut(obj, event, centerX) {
    ...
    $(obj).find('.hoverWrapper h3').stop().animate({
        opacity: '0'
    }, 300);
    $(obj).find('.hoverWrapper .enter-text').stop().animate({
        opacity: '0'
    }, 300);
}
...
function closeMenu() {
    ...
    $('#content').stop().show().animate({
        opacity: 1
    }, 800);
    $('#menu-container').stop().animate({
        left: '-' + ($('#menu-container').outerWidth()) + 'px'
    }, 400, 'easeOutQuad', function() {
        $('#menuOpener').show().animate({
            opacity: '1'
        });
...
function closeSubMenu(menuID) {
    if (menuID == undefined) {
        $('#mainmenu ul li ul').stop(true, true).animate({
            opacity: '0'
        }, 400, 'easeOutExpo', function() {
            $(this).hide();
        });
    } else {
        $('#mainmenu ul li:not([id=' + menuID + ']) ul').stop(true, true).animate({
            opacity: '0'
        }, 400, 'easeOutExpo', function() {
            $(this).hide();
        });
...
function menuItemOver(obj) {
    ...
    if (!mobileDevice)
        clearTimeout(menuTimer);
    $(obj).find('span').stop(true, true).animate({
        top: 75
    }, 0).animate({
        top: 0
    }, 600, 'easeOutBack');
...
function showMenu() {
    ...
    $('#menu-container').stop(true).animate({
        left: 0
    }, 600, 'easeOutExpo');
    $('#logo img').stop(true).animate({
        ...
        $('ul.menu > li').each(function(i, el) {
            $(el).find('> a > span').stop(true).delay(100 * i).animate({
                top: 0
            }, 600, 'easeOutBack');
            if (!mobileDevice) {
                $(el).find('> a').hover(function(event) {
                    $(this).parent().find('ul').stop().show().animate({
                        opacity: '1'
                    }, 600, 'easeOutExpo');
                    clearTimeout(subMenuTimer);
